...
description: Owns the technical implementation of high-fidelity analytics, team management, and learning gap visualization for FFA Advisors. Use when the user asks about advisor dashboards, student performance gaps, class leaderboards, engagement heatmaps, or assignment/goal-setting features.
---
...
interface PerformanceGap {
  ...
  mostMissedId?: string; // specific item ID consistently missed
}
```

### UI: The "Focus Area" Card

- Use **Theme.colors.accent** for highlight areas.
- Use **vibrant reds (#FF4444)** for critical weak spots.
- Include a "Generate Study Material" button that creates a custom study set for that weak area.

## Google Classroom Integration

- Use the `TeacherService.assignToGoogleClassroom` pattern.
- Ensure assignments include the `contestId` and `targetScore`.
