...
### 5. ID / Flashcard Study Modules

**Reference**: `app/practice/vet-science-id/flashcards.tsx`, `app/practice/floral-id/study.tsx`
**Flow**: Image/Term Display → Reveal Answer/Flip Card → Navigate
**Scoring**: None (Study Mode)
**Critical Notes**: MUST implement backward navigation (`handlePrev`) so users can review previous cards. Do not force forward-only navigation.

## Module Creation Checklist

1. [ ] Determine contest type (quiz / speech / role-play / document / flashcard)
2. [ ] Create folder: `app/practice/{contest-slug}/`
3. [ ] Create `index.tsx` with instructions matching official FFA rules
4. [ ] Create practice screen(s) following the appropriate template above (Ensure back navigation exists for flashcards/study modes)
