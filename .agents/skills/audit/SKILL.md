...
Runs the **Four Cs Audit** on the current Codex project. Reads (never writes) the project's operating manual, memory, skills, agents, MCPs, decisions, and references. Scores each of the Four Cs out of 25. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next-step commands.
...
**Operating manual:** `AGENTS.md` (root), `Codex.local.md` (gitignored).
**Memory:** `MEMORY.md` (root), `~/.Codex/projects/<id>/memory/MEMORY.md`, or `memory/` folder.
**Skills:** `.Codex/skills/*/SKILL.md` — count + frontmatter.
**Agents:** `.Codex/agents/*.md` — count + frontmatter.
**Connection mechanisms** (any of these = "reachable"):
- MCPs: `.mcp.json`, `.Codex/settings.json` (mcpServers key), `.Codex/settings.local.json`
- API scripts: `scripts/*.py|.js|.ts` documented in AGENTS.md
...
**Templates:** `templates/`, `.Codex/templates/`.
**Hooks / scheduled jobs:** `.Codex/settings.json` hooks key, or skill names matching `morning-*`, `weekly-*`, `daily-*`, `monthly-*`, `standup`.
...
|---|---|---|
| Operating manual exists and is substantive (>200 words) | 5 | Read AGENTS.md, count words |
| Identity / role / voice captured | 5 | AGENTS.md mentions who the user is + role/mission, OR `.Codex/rules/*.md` exists |
...
| Criterion | Points | How to detect |
|---|---|---|
| 3+ skills installed | 10 | Count `.Codex/skills/*/SKILL.md` |
...
| 1+ agent defined | 5 | Count `.Codex/agents/*.md` ≥ 1 |

#### Cadence (25 pts)

| Criterion | Points | How to detect |
|---|---|---|
| 1+ recurring/scheduled trigger | 10 | `.Codex/settings.json` hooks, OR skill name matches `morning-*` / `daily-*` / `weekly-*` / `monthly-*` / `standup` |
| Recent activity / usage signal | 10 | Files in `.Codex/skills/` modified within 30 days, OR `decisions/log.md` has entry within 30 days |
| Templates folder populated | 5 | `templates/` or `.Codex/templates/` has ≥1 file |
...
- **Need a new skill?** Recommend `skill-creator` (Anthropic) or `skill-builder` (if local), or "write SKILL.md at `.Codex/skills/<name>/SKILL.md` with YAML frontmatter."
- **Need to log a decision?** "Append to `decisions/log.md`."
- **Need to reach a tier-1 domain?** Prefer API+script (write `scripts/{tool}_api.py` + save `references/{tool}-api.md`). Recommend `Codex mcp add` only if no API path exists.
...
- **Need a recurring trigger?** "Add a hook to `.Codex/settings.json`, or write a skill named `daily-*` you run each morning."
...
- **Read-only by default.** Never modify AGENTS.md, memory, skills, or any project files. Only optional write is the audit report.
